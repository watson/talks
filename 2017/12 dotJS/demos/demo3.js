...
function onMsg (self, mm) {
  ...
  let display = 'ID      Flight    Alt    Spd  Hdg      Lat      Long  Msgs\n'
  Object.keys(aircrafts).sort().forEach(function (addr) {
    const mm = aircrafts[addr]
    display += printf('%06s  %8s  % 5s  % 3s  % 3s  %7.3f  %8.3f  %d\n',
      addr,
      mm.flight || '',
      mm.altitude || '',
      Math.round(mm.velocity || 0) || '',
      Math.round(mm.heading || 0) || '',
      (mm.lat || 0),
      (mm.lon || 0),
      mm.count)
  })
